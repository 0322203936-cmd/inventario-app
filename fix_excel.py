...
for i, row in enumerate(values):
    ...
    if isinstance(raw_reg, (int, float)):
        ...
        if dt.day in [6, 7, 8] and dt.month != dt.day:
            new_reg = dt.replace(month=dt.day, day=dt.month)
            new_row[0] = new_reg.strftime("%m/%d/%Y %H:%M")
            changed = True
        else:
            new_row[0] = dt.strftime("%m/%d/%Y %H:%M")
            changed = True
    ...
    if isinstance(raw_fecha, (int, float)):
        ...
        if dt.day in [6, 7, 8] and dt.month != dt.day:
            new_fecha = dt.replace(month=dt.day, day=dt.month)
            new_row[2] = new_fecha.strftime("%m/%d/%Y")
            changed = True
        else:
            new_row[2] = dt.strftime("%m/%d/%Y")
            changed = True
    ...
    if changed:
        print(f"Row {i}: {row[0]} -> {new_row[0]} | {row[2]} -> {new_row[2]}")

    fixed_values.append(new_row)

print("Ready to patch... uncomment patch logic to apply.")
