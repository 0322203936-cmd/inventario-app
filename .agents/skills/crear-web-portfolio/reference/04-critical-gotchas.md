...
### B.13 ❌ A broad descendant selector silently overrides a specific element's `display`

**Symptom:** a component that should be a tidy flex row (an odometer's digit
columns, a "name + verified badge" row, an inline stat) renders scattered,
stacked, or the wrong size — even though you clearly wrote `display:flex` on it.
Nothing in *its own* rule is wrong.

**Cause:** a nearby rule that styles a *sibling or label* with a bare descendant
type selector, e.g. `.card span { display:block; font-size:.7rem }`. Its
specificity **(0,1,1)** beats your element's single-class rule **(0,1,0)**, so it
quietly wins the `display` (and font-size) battle on every matching `<span>`
inside `.card` — including ones you never meant to touch (the odometer digits,
the verified badge's wrapper).

**Solution:** never style structural children with a bare descendant type
selector. Scope to a **direct child** or a **class**:

```css
/* ❌ clobbers every span under .card, including nested component internals */
.card span { display:block; font-size:.7rem }
/* ✅ only the labels you meant */
.card > .label { display:block; font-size:.7rem }
```

When a display bug makes no sense, **measure with `getComputedStyle`** (zoom-
independent and reliable) rather than trusting a zoomed screenshot: read the
element's computed `display`/`font-size` and you'll catch the override
immediately. This exact class of bug (broken rolling counter, verified badge on
its own line) is the most common cause of "looks buggy" in these builds.

### B.14 ❌ Global `img,svg,canvas{display:block}` reset makes an inline icon drop to its own line

**Symptom:** in a `[avatar] Name [✓]` row, the verified `<svg>` (or a small inline
icon) jumps to a second line the moment the row stops being a flex container.

**Cause:** the common reset `img,svg,canvas{display:block;max-width:100%}` makes
those elements block-level. Inside a **flex** parent that's harmless (flex items
ignore `display:block` for flow). But if the parent loses its `flex` — often via
the B.13 specificity trap above — the block-level `<svg>` forces its own line.

**Solution:** keep the row `display:flex; align-items:center` and make sure no
broader rule downgrades it (scope siblings per B.13). If you truly need the icon
inline in non-flex text, set `display:inline-block; vertical-align:middle` on that
icon explicitly.
